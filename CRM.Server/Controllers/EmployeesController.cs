...
namespace CRM.Server.Controllers
{
    ...
    public class EmployeesController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<Employee>>> GetEmployyes()
        {
        ...
        {
            _appDbContext.Employees.AddAsync(emp);
            await _appDbContext.SaveChangesAsync();
            return CreatedAtAction("GetEmployeeById", new { id = emp.Id }, emp);
        }
        //https://localhost:7133/api/Employees/1
        [HttpGet("{id}")]
        public async Task<ActionResult<IEnumerable<Employee>>> GetEmloyeesId(int id)
        {
            var emp = await _appDbContext.Employees.FindAsync(id);
            //var amp = await _appDbContext.Employees.FirstOrDefaultAsync(e=>e.Id == id); 
            if (emp == null)
            {
                return NotFound();
            }
        ...
        public async Task<ActionResult<Employee>> UpdateEmployeeById(int id, Employee emp)
        {
            ...
            try
            {
                await _appDbContext.SaveChangesAsync();//çalıştırılacak kod bıloğu
            }
            catch (Exception ex)
            {
